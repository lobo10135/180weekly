...
st.set_page_config(page_title="180's Weekly Scanner", layout="wide")
...
def get_eurostoxx50_tickers():
    ...
    # Die Tabelle mit den Komponenten finden (meistens die Tabelle mit der Überschrift "Ticker" oder "Name")
    target_df = None
    for table in tables:
    ...
    if target_df is None:
        # Fallback auf die üblicherweise relevante Tabelle (Index 3 oder ähnlich, oder Suche nach Spalten)
        target_df = tables[2] if len(tables) > 2 else tables[0]
        
    # Spalte mit Tickern identifizieren
    ticker_col = None
    ...
    if ticker_col is None:
        # Fallback: Versuche die Spalte zu nehmen, die typische Yahoo-Finance Kürzel enthält oder Index 1/2
    ...
    # Bereinigung für yfinance (z.B. Punkte durch Bindestriche ersetzen oder Leerzeichen entfernen)
...
def evaluate_stock_weekly(df_single, ticker, strategy_mode):
    ...
    try:
        ...
        # Resample auf Wochenbasis (W = Weekly), Aggregation für OHLC
        ...
        w = df_weekly.iloc[-1]       # Aktuelle Woche
        w_minus_1 = df_weekly.iloc[-2] # Vorwoche
...
with col1:
    run_sp_long = st.button("🟢 S&P500 Long (Weekly)", type="secondary")
with col2:
    run_sp_short = st.button("🔻 S&P500 Short (Weekly)", type="secondary")

# Zeile 2: Euro Stoxx 50 Buttons
st.subheader("Euro Stoxx 50 Scanner")
col_es1, col_es2 = st.columns(2)
with col_es1:
    run_estoxx_long = st.button("🟢 Euro Stoxx 50 Long (Weekly)", type="secondary")
with col_es2:
    run_estoxx_short = st.button("🔻 Euro Stoxx 50 Short (Weekly)", type="secondary")

# Zeile 3: DAX Buttons
st.subheader("DAX (Deutscher Aktienindex) Scanner")
col3, col4 = st.columns(2)
with col3:
    run_dax_long = st.button("🟢 Dax Long (Weekly)", type="secondary")
with col4:
    run_dax_short = st.button("🔻 Dax Short (Weekly)", type="secondary")

# Zeile 4: MDAX Buttons
st.subheader("MDAX Scanner")
col5, col6 = st.columns(2)
with col5:
    run_mdax_long = st.button("🟢 MDax Long (Weekly)", type="secondary")
with col6:
    run_mdax_short = st.button("🔻 MDax Short (Weekly)", type="secondary")
